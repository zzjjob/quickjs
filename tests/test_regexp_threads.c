...
static const char regexp_stress_source[] =
    ...
    "  re.compile('abcdef[0-9]+', 'i');\n"
    "  match = re.exec('00ABCDEF42');\n"
    "  if (!match || match[0] !== 'ABCDEF42')\n"
    "    throw new Error('ignore-case prefix mismatch');\n"
    "  re.compile('(^|x)abc', 'ig');\n"
    "  if ('00xABC xabc'.replace(re, 'X') !== '00X X')\n"
    "    throw new Error('ignore-case leading candidate mismatch');\n"
    "  re.compile('kilo', 'iu');\n"
    "  match = re.exec('00\\u212aILO');\n"
    "  if (!match || match[0] !== '\\u212aILO')\n"
    "    throw new Error('unicode fallback mismatch');\n"
